...
config = AppConfig()
detector = ShiTomasiDetector(config.feature_detector)

dust_sim = DustSimulator(config)

# Load a test image (replace with your capture_frame() or file path)
frame = cv2.imread(
    "/home/kyle/repos/Autonomous-Navigation/bbox_images/training_img_09.jpg"
)

gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
clean_features = detector.detect_features(gray)
print(
...
dust_features = detector.detect_features(dust_gray)
print(
